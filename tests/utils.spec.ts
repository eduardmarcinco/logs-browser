...
describe('Utils', () => {
  ...
  describe('parseTags', () => {
    it('should add BrowserLogs when no tags are passed', () => {
      const tags = utils.parseTags();
      expect(tags).toEqual('BrowserLogs');
    });

    it('should add BrowserLogs along with the string of tags', () => {
      const tags = utils.parseTags('tag');
      expect(tags).toEqual('BrowserLogs,tag');
    });

    it('should add BrowserLogs when passing an array of tags', () => {
      const tags = utils.parseTags(['tag1', 'tag2']);
      expect(tags).toEqual('BrowserLogs,tag1,tag2');
    });

    it('should pass BrowserLogs as a tag and show a console error when passing non string', () => {
      //@ts-ignore
      expect(() => utils.parseTags(123)).toThrowError();
    });

    it('should pass BrowserLogs as a tag and show a console error when passing an array with non string values', () => {
      //@ts-ignore
